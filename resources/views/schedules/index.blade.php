...
<head>
    ...
    <title>Schedule Management - Triconnect</title>
            ...
            <div class="container-fluid">
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <p class="text-muted">Manage school schedules, assign teachers to subjects and rooms</p>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <a href="{{ route('schedules.weekly') }}" class="btn btn-success">
                            <i class="fa fa-calendar-week"></i> Weekly View
                        </a>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        <div class="filter-section">
                            <form action="{{ route('schedules.index') }}" method="GET" class="row">
                                <div class="col-md-3">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select name="room" id="room" class="form-control">
                                        ...
                                        @foreach($rooms as $room)
                                            <option value="{{ $room->room_code }}" {{ request('room') == $room->room_code ? 'selected' : '' }}>
                                                {{ $room->name }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="col-12">
                        <div class="card schedule-card">
                            <div class="card-header">
                                <h5><i class="fa fa-calendar"></i> School Schedules</h5>
                            </div>
                            <div class="card-body">
                                @if (session('success'))
                                    <script>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped" id="schedules">
                                        <thead>
                                            <tr>
                                                <th>Teacher</th>
                                                <th>Subject</th>
                                        ...
                                        <tbody>
                                            @foreach ($schedules as $schedule)
                                                <tr>
                                                    <td>
                                                        <strong>{{ $schedule->teacher->firstname ?? 'N/A' }} {{ $schedule->teacher->lastname ?? 'N/A' }}</strong>
                                                        <br><small class="text-muted">{{ $schedule->teacher_staff_code }}</small>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="btn-group" role="group">
                                                            ...
                                                            <form action="{{ route('schedules.toggle-status', $schedule) }}" method="POST" style="display: inline;">
                                                                @csrf
                                                                <button type="submit" class="btn btn-sm btn-{{ $schedule->status === 'active' ? 'warning' : 'success' }}" title="{{ $schedule->status === 'active' ? 'Deactivate' : 'Activate' }}">
                                                                    <i class="fa fa-{{ $schedule->status === 'active' ? 'pause' : 'play' }}"></i>
                                                                </button>
                                                            </form>
                                                            <form action="{{ route('schedules.destroy', $schedule) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this schedule?')">
                                                                @csrf
                                                                @method('DELETE')
